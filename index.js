...
exports.getAdminUsers=onCall(async(req)=>{requireRole(req,['OWNER']);const users=(await db.ref('users').get()).val()||{},requests=(await db.ref('adminRequests').get()).val()||{};return {users:Object.fromEntries(Object.entries(users).filter(([,u])=>['ADMIN','SUB-ADMIN'].includes(u?.role))),requests:Object.fromEntries(Object.entries(requests).filter(([,r])=>r?.status==='PENDING'))};});
