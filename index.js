...
exports.getScopedFiles=onCall(async(req)=>{requireAuth(req);const all=(await db.ref('files').get()).val()||{};const out={};for(const [id,f] of Object.entries(all)){let ok=role(req)==='OWNER';if(role(req)==='STUDENT')ok=(!f.groupId&&!f.schoolId)||f.groupId===req.auth.token.groupId||f.schoolId===req.auth.token.schoolId;if(role(req)==='SUB-ADMIN')ok=(!f.groupId&&!f.schoolId)||await hasGroupScope(req.auth.uid,f.groupId)||await hasSchoolScope(req.auth.uid,f.schoolId);if(role(req)==='ADMIN')ok=(!f.groupId&&!f.schoolId)||await hasSchoolScope(req.auth.uid,f.schoolId);if(ok)out[id]=f;}return out;});
exports.getFileUrl=onCall(async(req)=>{requireAuth(req);const id=clean(req.data?.id);const f=(await db.ref(`files/${id}`).get()).val();if(!f)throw new HttpsError('not-found','الملف غير موجود.');let ok=role(req)==='OWNER';if(role(req)==='STUDENT')ok=f.groupId===req.auth.token.groupId||f.schoolId===req.auth.token.schoolId;if(role(req)==='SUB-ADMIN')ok=(!f.groupId&&!f.schoolId)||await hasGroupScope(req.auth.uid,f.groupId)||await hasSchoolScope(req.auth.uid,f.schoolId);if(role(req)==='ADMIN')ok=(!f.groupId&&!f.schoolId)||await hasSchoolScope(req.auth.uid,f.schoolId);if(!ok)throw new HttpsError('permission-denied','ليس لديك صلاحية لهذا الملف.');const [url]=await admin.storage().bucket().file(f.path).getSignedUrl({action:'read',expires:Date.now()+15*60*1000});return {url};});
