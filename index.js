...
async function userProfile(uid){
  const current=await db.ref(`users/${uid}`).get();
  if(current.exists())return current.val();
  const legacy=await db.ref(`Users/${uid}`).get();
  return legacy.exists()?legacy.val():null;
}
async function allUserProfiles(){
  const current=(await db.ref('users').get()).val()||{};
  const legacy=(await db.ref('Users').get()).val()||{};
...
exports.approveStudent=onCall(async(req)=>{
  ...
  const group=(await db.ref(`groups/${groupId}`).get()).val(); if(!group||group.schoolId!==schoolId)throw new HttpsError('invalid-argument','المجموعة لا تتبع المدرسة المحددة.'); await db.ref('students/'+uid).update({status:'ACTIVE',schoolId,groupId,updatedAt:now()}); await db.ref('users/'+uid).update({status:'ACTIVE',schoolId,groupId,updatedAt:now()}); await setClaims(uid,{role:'STUDENT',schoolId,groupId}); await writeAudit(req.auth.uid,'OWNER','APPROVE','STUDENT',uid,{status:'PENDING'},{status:'ACTIVE',schoolId,groupId}); return {ok:true};
});
exports.rejectStudent=onCall(async(req)=>{
  requireRole(req,['OWNER']); const uid=clean(req.data?.uid); if(!uid)throw new HttpsError('invalid-argument','معرف الطالب مطلوب.');
...
exports.getScopedStudents=onCall(async(req)=>{requireAuth(req);const r=role(req),all=(await db.ref('students').get()).val()||{};const out={};if(r==='OWNER')return all;for(const [id,s] of Object.entries(all)){let ok=false;if(r==='STUDENT')ok=id===req.auth.uid;else if(r==='SUB-ADMIN')ok=await hasGroupScope(req.auth.uid,s.groupId);else if(r==='ADMIN')ok=await hasSchoolScope(req.auth.uid,s.schoolId);if(ok)out[id]=s;}return out;});
exports.getNotifications=onCall(async(req)=>{requireAuth(req);const all=(await db.ref('notifications').get()).val()||{};const out={};for(const [id,n] of Object.entries(all)){if(role(req)==='OWNER'||n.targetUserId===req.auth.uid)out[id]=n;}return out;});
exports.getAdminUsers=onCall(async(req)=>{requireRole(req,['OWNER']);const users=await allUserProfiles(),requests=(await db.ref('adminRequests').get()).val()||{};return {users:Object.fromEntries(Object.entries(users).filter(([,u])=>['ADMIN','SUB-ADMIN'].includes(u?.role))),requests:Object.fromEntries(Object.entries(requests).filter(([,r])=>r?.status==='PENDING'))};});
exports.getScopedFiles=onCall(async(req)=>{requireAuth(req);const all=(await db.ref('files').get()).val()||{};const r=role(req);let studentScope={};if(r==='STUDENT')studentScope=(await db.ref(`students/${req.auth.uid}`).get()).val()||{};const groupId=studentScope.groupId||req.auth.token.groupId,schoolId=studentScope.schoolId||req.auth.token.schoolId;const out={};for(const [id,f] of Object.entries(all)){let ok=r==='OWNER';if(r==='STUDENT')ok=(!f.groupId&&!f.schoolId)||f.groupId===groupId||f.schoolId===schoolId;if(r==='SUB-ADMIN')ok=(!f.groupId&&!f.schoolId)||await hasGroupScope(req.auth.uid,f.groupId)||await hasSchoolScope(req.auth.uid,f.schoolId);if(r==='ADMIN')ok=(!f.groupId&&!f.schoolId)||await hasSchoolScope(req.auth.uid,f.schoolId);if(ok)out[id]=f;}return out;});
exports.getFileUrl=onCall(async(req)=>{requireAuth(req);const id=clean(req.data?.id);const f=(await db.ref(`files/${id}`).get()).val();if(!f)throw new HttpsError('not-found','الملف غير موجود.');const r=role(req);let studentScope={};if(r==='STUDENT')studentScope=(await db.ref(`students/${req.auth.uid}`).get()).val()||{};const groupId=studentScope.groupId||req.auth.token.groupId,schoolId=studentScope.schoolId||req.auth.token.schoolId;let ok=r==='OWNER';if(r==='STUDENT')ok=(!f.groupId&&!f.schoolId)||f.groupId===groupId||f.schoolId===schoolId;if(r==='SUB-ADMIN')ok=(!f.groupId&&!f.schoolId)||await hasGroupScope(req.auth.uid,f.groupId)||await hasSchoolScope(req.auth.uid,f.schoolId);if(r==='ADMIN')ok=(!f.groupId&&!f.schoolId)||await hasSchoolScope(req.auth.uid,f.schoolId);if(!ok)throw new HttpsError('permission-denied','ليس لديك صلاحية لهذا الملف.');const [url]=await admin.storage().bucket().file(f.path).getSignedUrl({action:'read',expires:Date.now()+15*60*1000});return {url};});
...
exports.createFileRecord=onCall(async(req)=>{requireRole(req,['OWNER']);const d=req.data||{};const name=clean(d.name),contentType=clean(d.contentType),path=clean(d.path),size=Number(d.size),schoolId=clean(d.schoolId),groupId=clean(d.groupId);const allowed=/^(application\/pdf|application\/msword|application\/vnd\.openxmlformats-officedocument\.wordprocessingml\.document|image\/(?:png|jpeg|webp|gif)|video\/(?:mp4|webm|quicktime))$/.test(contentType);if(!name||!path.startsWith('uploads/')||!allowed||!Number.isFinite(size)||size<=0||size>50*1024*1024)throw new HttpsError('invalid-argument','بيانات الملف غير مسموحة.');if(!/^uploads\/[A-Za-z0-9._\/-]+$/.test(path))throw new HttpsError('invalid-argument','مسار الملف غير صالح.');if(schoolId&&!await db.ref(`schools/${schoolId}`).get().then(x=>x.exists()))throw new HttpsError('invalid-argument','المدرسة غير موجودة.');if(groupId){const g=(await db.ref(`groups/${groupId}`).get()).val();if(!g||schoolId&&g.schoolId!==schoolId)throw new HttpsError('invalid-argument','المجموعة غير متوافقة.');}const id=db.ref('files').push().key;const x={name,contentType,size,path,schoolId:schoolId||null,groupId:groupId||null,uploadedBy:req.auth.uid,uploadedByRole:'OWNER',status:'ACTIVE',createdAt:now()};await db.ref(`files/${id}`).set(x);await writeAudit(req.auth.uid,'OWNER','UPLOAD','FILE',id,null,x);return {ok:true,id};});

exports.getBootstrapStatus=onCall(async()=>{
  const users=await allUserProfiles();
  const owner=Object.values(users).find(x=>x?.role==='OWNER');
  const initialized=(await db.ref('system/ownerInitialized').get()).val()===true;
  return {available:!owner&&!initialized};
});
exports.bootstrapOwner=onCall(async(req)=>{
  requireAuth(req);
  const users=await allUserProfiles();
  if(Object.values(users).some(x=>x?.role==='OWNER'))throw new HttpsError('already-exists','تمت تهيئة Owner بالفعل.');
  if((await db.ref('system/ownerInitialized').get()).val()===true)throw new HttpsError('already-exists','تم استخدام تهيئة Owner بالفعل.');
  if(await userProfile(req.auth.uid))throw new HttpsError('failed-precondition','هذا الحساب مرتبط بملف مستخدم بالفعل.');
  const lock=await db.ref('system/ownerInitialized').transaction(value=>value===true?value:true);
  if(!lock.committed||lock.snapshot.val()!==true)throw new HttpsError('aborted','تعذر حجز تهيئة Owner، حاول مرة أخرى.');
  try{
    await db.ref('users/'+req.auth.uid).set({name:req.auth.token.name||req.auth.token.email||'Owner',email:req.auth.token.email||'',role:'OWNER',status:'ACTIVE',createdAt:now(),updatedAt:now()});
    await setClaims(req.auth.uid,{role:'OWNER'});
    return {ok:true};
  }catch(error){
    await db.ref('system/ownerInitialized').remove();
    throw new HttpsError('internal','تعذر إنشاء Owner.');
  }
});
