(() => {
  ...
  const addInstallButton = () => {
    ...
    button.innerHTML = `
      <img class="install-app-icon" src="./IMG_20260923_173158_131_1790175684541.jpg" alt="" />
      <span class="install-app-copy"><strong>تثبيت التطبيق</strong><small>إضافة المنصة إلى الشاشة الرئيسية</small></span>
      <span class="install-app-arrow" aria-hidden="true">⇩</span>`;
  ...
  const lockAdminGradeEditor = () => {
    if (currentRole() !== "admin") return;
    const heading = [...document.querySelectorAll("h1")].find((node) =>
      text(node).includes("درجات أعمال السنة")
    );
    if (!heading) return;

    const page = heading.closest(".content-inner") || document;
    const subtitle = heading.parentElement?.querySelector("p");
    if (subtitle) subtitle.textContent = "عرض درجات الطلاب للقراءة فقط";

    page.querySelectorAll(".data-table input[type=\"number\"]").forEach((input) => {
      const value = input.value.trim() || "لم ترصد بعد";
      const display = document.createElement("strong");
      display.className = "admin-grade-readonly";
      display.textContent = value;
      input.replaceWith(display);
    });

    page.querySelectorAll(".data-table button").forEach((button) => {
      if (text(button).includes("حفظ")) {
        button.replaceWith(Object.assign(document.createElement("span"), {
          className: "badge neutral",
          textContent: "قراءة فقط"
        }));
      }
    });

    page.querySelectorAll(".data-table thead th").forEach((cell) => {
      if (text(cell) === "حفظ") cell.textContent = "الحالة";
    });
  };
